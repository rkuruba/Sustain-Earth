...
S3_BUCKET ='weather-data-hac-201-789'

# set defaults
DEFAULT_CITIES = "list" # choices are 'list' (returns all cities) or 'best' (returns city with closest temp to target temp)
DEFAULT_TARGET = 10 # Any int that is represented in tenths of mm
DEFAULT_DATE_HISTORY = 3 # defaults to 3 days from current day in SQL query
DEFAULT_MIN_LOOKBACK = 2
# number of retries
RETRY_COUNT = 15
...
def lambda_handler(event, context):
    ...
    query = f"""SELECT city, avg(CAST(data_value as INTEGER)) as prcp FROM "{STADIUM_TABLE_NAME}" as stadium
    ...
    while i < num_cities:
        # Pull out the station city and station avg precipitation from the json returned from query
        station_city = result['ResultSet']['Rows'][i]['Data'][0]['VarCharValue']
        station_prep = int(float(result['ResultSet']['Rows'][i]['Data'][1]['VarCharValue']))
        
        # the delta from target shows how far (in tenths of a degree) we are from the target precipitation
        delta_from_target = target - station_prep
        turn_off_sprincler = False
        if (station_prep > target):
             turn_off_sprincler = True
        
        # save it in a new dict. Station[<City Name>] = [ degree delta from target, avg temp of city]
        stations[station_city] = [ turn_off_sprincler, station_prep ] 
        i = i+1
        
    sorted_stations = sorted(stations.items(), key=operator.itemgetter(1))
    stations_dict = collections.OrderedDict(sorted_stations)   
    best_city = 'not_working'
